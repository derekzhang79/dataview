...
def generate_price1(data, input_column='bidprice10', multiplier=1.1):
    """通过bidprice10列生成price1列数据"""
    # 确保输入列存在
    if input_column not in data.columns:
        print(f"警告: 输入文件中不存在{input_column}列，price1将使用默认值")
        return [0] * len(data)
...
def generate_price2(data, input_column='bidprice9', multiplier=1.1):
    """通过bidprice9列生成price2列数据"""
    # 确保输入列存在
    if input_column not in data.columns:
        print(f"警告: 输入文件中不存在{input_column}列，price2将使用默认值")
        return [0] * len(data)
...
def generate_price3(data, column1='bidprice9', column2='bidprice10'):
    """通过bidprice9和bidprice10列生成price3列数据"""
    # 确保输入列存在
    if column1 not in data.columns or column2 not in data.columns:
        ...
        print(f"警告: 输入文件中不存在{', '.join(missing_columns)}列，price3将使用默认值")
        return [0] * len(data)
    
    # 处理可能的缺失值，使用0代替
    data_copy = data.copy()
    data_copy[column1] = data_copy[column1].fillna(0)
    data_copy[column2] = data_copy[column2].fillna(0)
    
    # 生成price3列（可以根据需要调整计算规则）
    return (data_copy[column1] + data_copy[column2]) / 2


def generate_price4(data, input_column='price', multiplier=1.2):
...
def generate_price5(data, input_column='price', multiplier=0.8):
